jQuery.fn.jRotator = function(prm){
	...
	$jR.prepend($jR_images).prepend($jR_points).prepend(!prm.nothumb?$jR_thumbs:'').prepend($L).prepend($R).prepend($jR_timeLine.css('background-color',prm.timeLineColor));
	...
	$jR_points.css({'margin':(h-10-pH)+'px 0 0 '+pL+'px','width':'auto'});
	$L.css('margin',(h/2-aH/2)+'px 0 0 '+5+'px');
	$R.css('margin',(h/2-aH/2)+'px 0 0 '+(w-5-aW)+'px');
	if(!prm.nothumb)
	{
		var tL;
		switch(prm.pAlign)
